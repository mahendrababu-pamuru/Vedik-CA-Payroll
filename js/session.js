async function requirePayrollSession(){if(!window.sb)return null;const {data:{session}}=await sb.auth.getSession();if(!session){location.href='index.html';return null}
 ...
 if(!adminError&&isAdmin===true){window.payrollSession={session,tenantId:null,role:'platform_admin',tenantName:'Platform Administration'};return window.payrollSession}
