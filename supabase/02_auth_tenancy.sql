...
create table if not exists departments(id uuid primary key default gen_random_uuid(),tenant_id uuid not null references tenants(id) on delete cascade,name text not null,status text not null default 'active' check(status in('active','inactive')),created_at timestamptz default now(),unique(tenant_id,name));
create table if not exists designations(id uuid primary key default gen_random_uuid(),tenant_id uuid not null references tenants(id) on delete cascade,name text not null,status text not null default 'active' check(status in('active','inactive')),created_at timestamptz default now(),unique(tenant_id,name));
...
create index if not exists departments_tenant_idx on departments(tenant_id,status);
create index if not exists designations_tenant_idx on designations(tenant_id,status);
...
alter table tenants enable row level security;alter table user_profiles enable row level security;alter table platform_admins enable row level security;alter table tenant_members enable row level security;alter table roles enable row level security;alter table subscriptions enable row level security;alter table departments enable row level security;alter table designations enable row level security;
...
drop policy if exists departments_tenant_access on departments;create policy departments_tenant_access on departments for all to authenticated using(is_platform_admin() or is_tenant_member(tenant_id)) with check(is_platform_admin() or is_tenant_member(tenant_id));
drop policy if exists designations_tenant_access on designations;create policy designations_tenant_access on designations for all to authenticated using(is_platform_admin() or is_tenant_member(tenant_id)) with check(is_platform_admin() or is_tenant_member(tenant_id));
